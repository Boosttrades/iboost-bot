...
async def username_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=ADMIN_USER_ID, text=admin_message, reply_markup=keyboard)
    
    await update.message.reply_text(
        f"🎉 Account created successfully!\n\nWelcome, {username}!\nYou can now start trading gift cards.",
        reply_markup=get_main_keyboard()
    )
    return ConversationHandler.END
...
async def profile_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("💰 Withdraw", callback_data="withdraw_profile")]])
...
async def card_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"💳 Amount: ${amount:.2f}\n\nPlease upload a clear image of your gift card:",
        reply_markup=get_home_back_keyboard()
    )
...
async def card_image_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text:
        if update.message.text == "🏠 Home":
            ...
            await update.message.reply_text("Please enter the gift card amount (in USD):")
            return CARD_AMOUNT_INPUT
    
    if not update.message.photo:
        await update.message.reply_text("❌ Please upload an image of your gift card:")
        return CARD_IMAGE_UPLOAD
    ...
    await context.bot.send_photo(chat_id=ADMIN_USER_ID, photo=file_id, caption=admin_message, reply_markup=keyboard)
...
async def withdraw_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user['withdrawal_cooldown']:
        ...
        if datetime.now() < cooldown_time:
            ...
            await update.message.reply_text(
                f"⏳ Withdrawal Cooldown Active\n\nYou cannot withdraw for another {hours} hours.\nPlease try again later.",
                reply_markup=get_main_keyboard()
            )
            return ConversationHandler.END
...
async def withdraw_source_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎁 Referral Bonus":
        if float(user['referral_bonus']) > 0:
            ...
            update_user(user_id, balance=new_balance, referral_bonus=0)
            await update.message.reply_text(
                f"✅ Referral Bonus Transferred!\n\n${float(user['referral_bonus']):.2f} added to your balance.\nNew Balance: ${new_balance:.2f}",
                reply_markup=get_main_keyboard()
            )
        else:
            await update.message.reply_text("❌ No referral bonus available.", reply_markup=get_main_keyboard())
        return ConversationHandler.END
    
    if text == "💵 Balance":
        if float(user['balance']) < MIN_WITHDRAWAL:
            await update.message.reply_text(
                f"❌ Insufficient Balance\n\nMinimum withdrawal: ${MIN_WITHDRAWAL}\nYour balance: ${float(user['balance']):.2f}",
                reply_markup=get_main_keyboard()
            )
            return ConversationHandler.END
...
async def withdraw_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"💰 Amount: ${amount:.2f}\n\nPlease enter your payout details ({context.user_data['withdraw_method']}):",
        reply_markup=get_home_back_keyboard()
    )
    return WITHDRAW_DETAILS_INPUT
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("activate_special_"):
        ...
        update_user(user_id, special_offer_status="Active", special_offer_expiry=expiry_date)
        ...
        await query.edit_message_text(query.message.text + "\n\n❌ Select decline reason:", reply_markup=keyboard)
        ...
        await context.bot.send_message(
            ...
            text=f"❌ Trade Declined\n\nTrade ID: {trade_id}\nReason: {reason_text}\n\nPlease submit a new trade with a valid card."
        )
        ...
        if reason_type == "insufficient":
            ...
            await context.bot.send_message(
                chat_id=user_id,
                text=f"⚠️ System Error Compensation\n\nWe apologize for the inconvenience.\n$10 has been credited to your account.\nNew Balance: ${new_balance:.2f}"
            )
...
async def admin_credit_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    update_user(trade['user_id'], balance=new_balance, total_trades=new_total_trades)
    update_trade(trade_id, credit_amount=final_amount, special_offer_applied=1 if special_applied else 0, status="Approved")
    ...
    if user['referred_by']:
        ...
        if referrer and user['total_trades'] == 0:
            ...
            update_user(user['referred_by'], referral_bonus=new_ref_bonus, referral_count=new_ref_count)
            
            await context.bot.send_message(
                chat_id=user['referred_by'],
                text=f"🎉 Referral Bonus Earned!\n\nYour referral {user['username']} completed their first trade!\n${REFERRAL_BONUS} added to your referral bonus."
            )
    ...
    if admin_msg_id and admin_chat_id:
        try:
            await context.bot.edit_message_text(
                ...
                text=f"✅ Trade Approved\n\nTrade ID: {trade_id}\nCredited: ${final_amount:.2f}" + (f" (2x Special Offer)" if special_applied else "")
            )
        except:
            pass
...
# ============================================
# MAIN FUNCTION - WEBHOOK VERSION FOR RENDER
# ============================================

def main():
    """Start the bot with webhooks for Render hosting"""
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CREDIT_AMOUNT_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_credit_input)],
            WITHDRAW_AMOUNT_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, withdraw_method_selected)],
            WITHDRAW_DETAILS_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, withdraw_details_input)],
        },
    ...
    if not WEBHOOK_URL:
        ...
        logger.error("Set it to: https://your-service-name.onrender.com")
        return
